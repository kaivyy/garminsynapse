...
- **MCP Server Upgrade**:
  - Integrated `nap_duration_mins` directly into `get_live_metrics` MCP tool output.
  - Enhanced `get_daily_summary` and `get_sleep_analysis` tools with automatic session restoration and step count fallback.
